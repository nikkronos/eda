...
- При ДОБАВЛЕНИИ (закупка, «купил X») сопоставляй покупку с существующими позициями
  холодильника, включая помеченные «кончилось». Существующее имя используй, только
  если это ТОТ ЖЕ продукт под другим названием: «филе грудки цыплят» = «курица»,
  «помидоры черри» = «черри-помидоры». РАЗНЫЕ продукты одной категории — раздельные
  позиции: обычные помидоры ≠ черри-помидоры, рожки ≠ спагетти, обычные спагетти ≠
  гречневые. Сомневаешься, тот же ли продукт, — создай отдельную позицию: дубль
  виден и легко сливается, а ложное слияние портит данные незаметно.
