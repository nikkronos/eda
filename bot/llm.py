...
- Сырьё и ГОТОВАЯ ЕДА — разные позиции. Сваренное/приготовленное, что можно есть
  сразу, веди отдельной позицией с пометкой «(готово)»: «булгур 2 пакетика + 1
  готовый» → булгур в пакетиках set 2 И «булгур (готово)» add 1 порция. «Сварил/
  приготовил X» — спиши сырьё, добавь «X (готово)». В отчёте о еде списывай
  готовую позицию, если она есть, а не сырьё. Сырая заготовка (порезанное,
  замаринованное) — та же позиция сырья, не отдельная.
...
- Списывай консервативно. Штучное, съедаемое целиком (яйца, огурцы, бананы,
  яблоки) — op="subtract" с числом штук. Продукт, хранящийся в граммах, списывай
  в граммах или без qty — никогда не в штуках.
...
- Обрывочные телеграфные реплики без явного факта («Овощи ещё», «и сыр», «ага»,
  ...
  осталось, добавь, вскрыл), количество, ЛИБО оператор «минус X» / «плюс X»:
  «минус огурец» → subtract (1 шт для штучного, иначе без qty), «плюс молоко» →
  add. Обрывок без таких сигналов может продолжать соседнее сообщение, которого
  ты не видишь, — не догадывайся за автора.
